...
export const Form_2: FC<FormProps> = ({ direction, className, ...formProps }) => {
  const styles = useStyles({ direction })
  ...
  return (
    <FormContext.Provider value={{ direction }}>
      <form
        {...formProps}
        className={combineClasses([styles.form_2, className])}
      />
    </FormContext.Provider>
  )
}
export const HorizontalForm_2: FC<HTMLProps<HTMLFormElement>> = ({
  children,
  ...formProps
}) => {
  return (
    <Form_2 direction="horizontal" {...formProps}>
      {children}
    </Form_2>
  )
}
...
> = ({ children, title, description, classes = {} }) => {
  ...
  return (
    <div className={combineClasses([styles.formSection_2, classes.root])}>
      <div
        className={combineClasses([
          classes.sectionInfo,
          styles.formSectionInfo,
...
const useStyles = makeStyles((theme) => ({
  ...
  form_2: {
    ...
    flexDirection: "column",
    gap: ({ direction }: FormContextValue = {}) =>
      direction === "horizontal" ? theme.spacing(2) : theme.spacing(5),
  ...
  formSection_2: {
    ...
    alignItems: "flex-start",
    gap: ({ direction }: FormContextValue = {}) =>
      direction === "horizontal" ? theme.spacing(5) : theme.spacing(3),
    flexDirection: ({ direction }: FormContextValue = {}) =>
      direction === "horizontal" ? "row" : "column",
