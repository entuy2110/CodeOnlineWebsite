...
const loadInitialAuthData = async (): Promise<AuthData> => {
  ...
  const authenticatedUser_2 = await API.getUser()

  if (authenticatedUser && authenticatedUser_2) {
...
const loginBySignUp = async (): Promise<AuthenticatedData> => {
  const [user, permissions, user_2] = await Promise.all([
    API.getAuthenticatedUser(),
    API.checkAuthorization({
      checks: permissionsToCheck,
  ...
  createMachine(
    {
      ...
      schema: {
        ...
        services: {} as {
          ...
          loginBySignUp: {
            data: Awaited<ReturnType<typeof loginBySignUp>>
          }
          forgotPassword: {
    ...
    },
    {
      services: {
        ...
        updateProfileV2: async (_, { data }) => {
          const res = await API.putUser(data.fullname)
          if (res) {
        ...
        updateUserV2: async (_, { data }) => {
          return data
        },
        getServer: async (context) => {
          await API.getServer()
          if (isAuthenticated(context.data)) {
            await API.getVisitor(context.data.user_2.id, context.data.user_2.fullname)
          }
        }
      },
      actions: {
